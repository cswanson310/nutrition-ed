module QuestionHelper
  def next_question_url
    ...
    next_q = possible_questions.select{|q| q.difficulty >= worst_skill_level - 1}.sample
    if next_q then
      return question_path(next_q)
    else
      return question_path(possible_questions.select{|q| q.difficulty < worst_skill_level}.sample)
    end
  end
